...
function createUI() {
    const createTemplate = ({id, phone, firstName, lastName}) => {
        ...
        <div class="text-black"><b>#${id} ${firstName} ${lastName}</b></div>
         <div class="d-flex justify-content-end gap-1">
            <button class="btn btn-success btn-sm"><i class="bi bi-telephone-outbound"></i></button>
            <button class="btn btn-danger btn-sm" data-remove-contact><i class="bi bi-trash3"></i></button>
         </div>
        `.trim();
    ...
    const renderContact = (template) => {
        ...
        const contactList = document.querySelector(`[data-contacts-list]`);
        contactList.prepend(template)
    }
